...
- **Advanced Security, Compliance & Governance**: Establishes an immutable, tamper-evident security audit logging infrastructure with cryptographic SHA-256 hash chaining (`previous_hash` + `event_hash`). Tracks administrative actions, authentication/security events, role changes, privilege updates, sensitive business data access, and API access denials (`/api/v1/security/`). Manages security threat investigations and incident lifecycles (`OPEN`, `INVESTIGATING`, `CONTAINED`, `RESOLVED`, `DISMISSED`). Evaluates configurable deterministic security policies and threshold rules. Generates audit integrity verification probes and enterprise compliance reports in JSON and CSV export formats. Recursively masks passwords, tokens, API keys, and credentials (`********`). Security and compliance auditing NEVER mutates core operational business state.
